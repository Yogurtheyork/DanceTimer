# Dance Timer — 街舞定時錄影 iOS 作業

原生 SwiftUI + AVFoundation，最低 iOS 17，第一版僅支援 iPhone 直向、後置相機。

流程：設定 40／60／自訂 1–600 秒 → 設定就位時間 → 按開始 → 就位倒數 → MC 節奏「5、4、321」→ 錄影並收錄現場聲音 → 到時自動停止 → 回放／儲存至相簿。

5、4 各 1 秒；3、2、1 各 0.4／0.5／0.7 秒，預設整段 MC 倒數 3.5 秒。倒數是大字搭配系統提示音，尚非真人 MC 語音，也不與音樂 BPM 同步。準備與 MC 倒數不計入錄影長度。

## 開發方式

1. Windows / WSL / Linux：編輯 `DanceTimer/Sources`、文件，使用 Git / GitHub 交接。
...
- 不接 Spotify、不登入、不使用廣告、分析 SDK 或網路上傳。Documents 可能由 iOS 隨裝置備份。
