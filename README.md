...
| `main`（最新） | 第一次開啟介紹頁：`AsyncImage` 網路照片＋`TimerCamLogo` 本機圖片 |
...
- `TimerCamApp.swift`：App 入口、註冊字型、`Font.app(_:size:)` 字型輔助。
- `ContentView.swift`：全螢幕相機介面（左下歷史錄影、中間錄影鍵、右下設定）、設定頁與回放；首次開啟時以 `fullScreenCover` 顯示介紹頁。
